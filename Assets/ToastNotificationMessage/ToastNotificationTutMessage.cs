...
/// </summary>
public class ToastNotificationTutMessage : MonoBehaviour
{
    [HideInInspector]
    ...
    void Start()
    {
        messageTime = messageTime <= -1 ? ToastNotificationTut.minimumMessageTime : messageTime;
    ...
    void FixedUpdate()
    {
        ...
        if (ToastNotificationTut.isStoped == true)
            return;
        ...
        if( messageTime != 0)
        {
            if (timeElapsed > messageTime)
            {
                ...
                if (ToastNotificationTut.isCanvasGroup)
                    ToastNotificationTut.isHiding = true;
                else
                    ToastNotificationTut.Hide();
            }
    ...
    {
        if (ToastNotificationTut.hideOnClick)
            ToastNotificationTut.Hide();
    }
